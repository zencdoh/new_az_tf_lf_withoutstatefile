...
resource "azurerm_virtual_machine" "weblt02" {
    ...
    location = "West US"
    resource_group_name = "${azurerm_resource_group.lt.name}"
    network_interface_ids = ["${azurerm_network_interface.ltwebpudinter2.id}"]
    vm_size = "Standard_A0"
...
resource "azurerm_public_ip" "ltweb01pub2" {
    name = "ltweb01pub2"
    location = "West US"
    resource_group_name = "${azurerm_resource_group.lt.name}"
    public_ip_address_allocation = "static"
...
resource "azurerm_network_interface" "ltwebpudinter2" {
    name = "ltwebpudinter2"
    location = "West US"
    resource_group_name = "${azurerm_resource_group.lt.name}"

    ip_configuration {
        name = "ltconfiguration2"
        subnet_id = "${azurerm_subnet.public.id}"
        private_ip_address_allocation = "dynamic"
        public_ip_address_id = "${azurerm_public_ip.ltweb01pub2.id}"
    }
}

resource "azurerm_dns_a_record" "azurelt_a_web_pub2" {
   name = "web_pub2"
   zone_name = "${azurerm_dns_zone.azurelt.name}"
   resource_group_name = "${azurerm_resource_group.lt.name}"
   ttl = "300"
   records = ["${azurerm_public_ip.ltweb01pub2.ip_address}"]
}
